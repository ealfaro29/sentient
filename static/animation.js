...
const Animation = {
  ...
  particlesExplodeThenKill: function(ms) {
    ...
    // Si no hay sketch (p.ej. en fallback), solo devuelve una promesa resuelta
    if (!this.sketch || this.canvasRemoved) { // <--- Modificado: chequear también si ya fue removido
  ...
  createAndFlyIcon: function(src, fromRect, targetSizePx, pill) {
    ...
    return new Promise(res => {
      img.addEventListener('transitionend', () => {
        // Prevenir que un icono vuele si el 'pill' ya no existe
  ...
  runMorph: async function(seed, isFallback = false) {
    ...
    // ===== INICIO DE LA CORRECCIÓN (ReferenceError) =====
    // Definir 'pill' y 'fusion' aquí para que estén en scope
    const pill = _app.els.morphPill, fusion = _app.els.fusion;
    if (!pill || !fusion) return;
    // ===== FIN DE LA CORRECCIÓN =====

    // Si es la primera vez (isFallback=false) y el canvas fue removido, lo recreamos.
    ...
    // Limpiar clases de pulso anteriores
    pill.classList.remove('is-pulsing');

    let flyPromise = Promise.resolve();
    
    if (!isFallback) {
      // 1. PRIMERA VEZ: Animación de partículas e icono
    ...
    // Si es fallback, no hacemos nada de esto (sin partículas, sin icono volador)
    ...
    // Añadir pulso siempre
    pill.classList.add('is-pulsing');

    // 1. Iniciar la llamada al API (sin await)
    const apiPromise = DataHandler.scrapeContent(_app, seed);

    // 2. Esperar a que la API se complete.
    try {
        const data = await apiPromise; 
        
        // 3. Éxito:
        pill.classList.remove('is-pulsing'); // Quitar pulso

        // 4. Ejecutar explosión (éxito)
        await this.particlesExplodeThenKill(this.EXP_MS);

        // 5. Data Handler finaliza la UI y hace el reveal
        await DataHandler.finalizeUi(_app, seed, data); 
        
        // 6. Transición final de UI
        ...
        pill.classList.remove('is-pulsing'); // Quitar pulso
        // pill ya está definido en el scope superior
        if (pill) pill.style.display = 'none'; 
