...
const App = {
    state: {
        ...
        data: {
            A: { title: 'READY', subtitle: 'Paste URL...', bg: 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=1080', tag: 'NEWS', layout: 'layout-standard' },
            B: { title: 'READY', subtitle: 'Paste URL...', bg: 'https://images.unsplash.com/photo-1557683316-973673baf926?q=80&w=1080', tag: 'FEATURE', layout: 'layout-centered' },
            C: { title: 'READY', subtitle: 'Paste URL...', bg: 'https://images.unsplash.com/photo-1504805572947-34fad45aed93?q=80&w=1080', tag: 'VIRAL', layout: 'layout-bold' }
        }
    },
    ...
    cacheDOM() {
        ...
        this.els={ urlIn:$('urlInput'), scrapeBtn:$('scrapeBtn'), editor:$('editorPanel'), dlBtn:$('dlBtn'), titleIn:$('titleInput'), subIn:$('subInput'), layoutSel:$('layoutSelector'), themeSel:$('themeSelector'), imgUrl:$('imgUrlInput'), imgFile:$('imgFileInput'), imgThumb:$('activeThumb'), btnA:$('btnVarA'), btnB:$('btnVarB'), btnC:$('btnVarC'), mockA:$('mockA'), mockB:$('mockB'), mockC:$('mockC') };
    },
    ...
    bindEvents() {
        ...
        ['A','B','C'].forEach(v => this.els[`btn${v}`].onclick=()=>this.switchVar(v));
    ...
    switchVar(v) {
        this.state.active=v;
        ['A','B','C'].forEach(x => { this.els[`btn${x}`].classList.toggle('active',x===v); this.els[`mock${x}`].classList.toggle('inactive',x!==v); });
        const d=this.state.data[v]; this.els.titleIn.value=d.title; this.els.subIn.value=d.subtitle; this.els.layoutSel.value=d.layout; this.updateThumb();
    },
    ...
    render(target=null) {
        ...
        (target?[target]:['A','B','C']).forEach(v=>{
            const c=document.getElementById(`card${v}`); const d=this.state.data[v];
            c.querySelector('.card-bg').src=d.bg; c.querySelector('.card-overlay').style.background=theme.overlay;
    ...
    async scrape() {
        ...
        try {
            ...
            if(d.ai_variants) {
                this.state.data.A.title=d.ai_variants.variant_a.title.toUpperCase(); this.state.data.A.subtitle=d.ai_variants.variant_a.subtitle;
                this.state.data.B.title=d.ai_variants.variant_b.title.toUpperCase(); this.state.data.B.subtitle=d.ai_variants.variant_b.subtitle;
                this.state.data.C.title=d.ai_variants.variant_c.title.toUpperCase(); this.state.data.C.subtitle=d.ai_variants.variant_c.subtitle;
            } else {
                const t=d.original.title.substring(0,40).toUpperCase(); const s=d.original.subtitle.substring(0,80);
                ['A','B','C'].forEach(v=>{this.state.data[v].title=t;this.state.data[v].subtitle=s});
            }
    ...
    async download(v) {
        const n=document.getElementById(`card${v}`); const btn=this.els.dlBtn; const og=btn.innerHTML; btn.innerHTML='SAVING...';
        n.style.transform='none'; try{const d=await htmlToImage.toPng(n,{width:1080,height:1350,style:{transform:'none'},useCORS:true,pixelRatio:1});const a=document.createElement('a');a.download=`Sentient_V14_${v}.png`;a.href=d;a.click();}catch(e){alert('Error saving');}finally{this.fitCards();btn.innerHTML=og;}
    }
};
App.init();